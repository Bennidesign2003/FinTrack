...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const rows: any = await db
      .select({ month: sql`strftime('%Y-%m', ${invoices.createdAt})`, total: sql`COALESCE(SUM(${invoices.totalAmount}),0)` })
      .from(invoices)
    ...
    return res.status(500).json({ error: 'Fehler beim Abrufen der Umsatzdaten', details: String(error) })
  }
}
